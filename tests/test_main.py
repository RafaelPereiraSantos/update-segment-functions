...
@patch('src.main.get_changed_files')
@patch('src.main.read_config_file')
@patch('src.main.read_raw_string_file')
@patch('src.main.update_segment_function')
@patch('src.main.validate_settings_payload')
...
def test_main_updates_changed_function(mock_env, mock_validate_payload, mock_update, mock_read_raw, mock_read_config, mock_changed_files):
    def env_get(key, default=''):
        env = {
            'INPUT_CONFIGURATION-FILE-PATH': 'config.yaml',
            'INPUT_SEGMENT-TOKEN': 'fake-token',
            'INPUT_TRUNK-BRANCH': 'main',
